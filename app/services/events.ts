// // app/(monitor)/(tabs)/api/events.ts
...
// app/(monitor)/(tabs)/api/events.ts
import { EventType } from "@/app/utils/Event";
import { db } from "@/configs/FirebaseConfig";
import {
  addDoc,
  collection,
...
/** 🔍 Get single event */
export const getEvent = async (id: string): Promise<EventType | null> => {
...
/** ❤️ Interested users (unchanged) */
// export const markInterested = async (eventId: string, user: any) => {
//   const userRef = doc(db, "events", eventId, "interested", user.uid);
//   const existing = await getDoc(userRef);
...
export const getInterestedUsers = async (eventId: string) => {
  const snap = await getDocs(collection(db, "events", eventId, "interested"));
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
};
...
export const listenInterestedCount = (
  ...
  return onSnapshot(collection(db, "events", eventId, "interested"), (snap) =>
    callback(snap.size)
  );
};

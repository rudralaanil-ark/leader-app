...
export default function AdminNewsContent({
  ...
  /* LOAD MONITOR MAP (ONLY FOR AVATAR FALLBACK) */
  useEffect(() => {
    (async () => {
      const map = await getMonitorMap();
      setMonitorMap(map);
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    const creatorRole = item.createdByRole || "";

    const avatar = monitorMap[item.createdBy]?.profileImage || null;
        ...
        <View style={styles.bottomBox}>
          ...
          <View style={styles.footerRow}>
            ...
            <View style={styles.createdByRow}>
              ...
              <View>
                <Text style={styles.createdByName}>{creatorName}</Text>
                {creatorRole ? (
                  <Text style={styles.createdByRole}>

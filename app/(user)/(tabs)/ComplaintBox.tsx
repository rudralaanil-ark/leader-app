...
export default function ComplaintBox() {
  ...
  const validateForm = () => {
    if (!title.trim()) return showToast("Enter complaint title.");
    if (!description.trim()) return showToast("Enter a description.");
    if (!phone.trim()) return showToast("Enter phone number.");
    if (!place.trim()) return showToast("Enter place/location.");
    return true;
  };
  ...
  const handlePickMedia = async (source: "camera" | "library") => {
    try {
      ...
      if (perms.status !== "granted") {
        Alert.alert("Permission Required", "Please grant media permissions.");
        return;
      }
  ...
  const handleSubmit = async () => {
    if (!user?.uid || !user.fullName) {
      return showToast("Login required.");
    }
    if (!validateForm()) return;
    ...
    try {
      ...
      for (const m of media) {
        const res = await uploadImageToCloudinary(m.uri);
        if (res?.url) {
          uploaded.push({ type: m.type, url: res.url });
        }
      }
  ...
  const renderItem = ({ item }: { item: ComplaintDoc }) => {
    ...
    // Show "New Reply" dot only if:
    const showUnread = !item.isReadByUser;

    const latestActivity = item.replies?.length
      ? safeDate(item.replies[item.replies.length - 1].createdAt)
      : created;

    const lastLabel = item.replies?.length ? "Last Reply" : "Created";
    ...
    return (
      <Pressable
        ...
        onPress={() =>
          router.push({
            pathname: "/(shared)/complaints/ComplaintDetails",
            params: { id: item.id },
          })
        }
      >
        ...
        <View style={styles.rowBetween}>
          ...
          <StatusBadge status={item.status} viewerRole="user" />
        </View>

        <Text style={styles.cardMeta}>
          {lastLabel}: {formatTimeAgo(latestActivity)}
        </Text>
        ...
        {/* Unread alert for user */}
        {showUnread && (
          <View style={styles.newRow}>
            <View style={styles.newDot} />
            <Text style={styles.newText}>New Reply</Text>
          </View>
        )}
      ...
      <View style={styles.tabs}>
        {(["new", "list"] as const).map((tab) => (
          ...
          >
            ...
            >
              {tab === "new" ? "New Complaint" : "My Complaints"}
            </Text>
          </Pressable>
        ))}
      </View>
